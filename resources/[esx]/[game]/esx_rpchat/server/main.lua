...
AddEventHandler('sendTwtServer', function(message)
	...
	local mamhajsikbejbe33 = xPlayer.getAccount('bank').money
		...
		if mamhajsikbejbe33 > 100 then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, '~y~Pobrano opłatę~r~ 100$')
			TriggerClientEvent('chatMessage', -1, "🕊️ @" ..name.firstname..' '..name.lastname, {30, 144, 255}, message)
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, '~y~Nie masz siana w banku')
		end
	end)
...
AddEventHandler('senddarkServer', function(message)
	...
	if mamhajsikbejbe > 500 then
		TriggerClientEvent("pokaDw", -1, -1, message)
		xPlayer.removeAccountMoney('bank', 500)

		TriggerClientEvent('esx:showNotification', xPlayer.source, '~y~Pobrano opłatę~r~ 500$')
	else
		TriggerClientEvent('esx:showNotification', xPlayer.source, '~y~Nie masz siana w banku')
	end
end)
...
function opissunio(hook,message,color)
    local e123asyadmin21 = 'https://discord.com/api/webhooks/827250174590058516/A3nR7mw1SUzTfXcEi-C_9g8K6vSzfCgQserFOGyFBhguIs7t_P-gexCFTJlARHzsx2wN'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
