...
Citizen.CreateThread(function ()
	...
	ESX.TriggerServerCallback('esx_vehicleshop1:getCategories', function (categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_vehicleshop1:getVehicles', function (vehicles)
		Vehicles = vehicles
	end)
...
RegisterNetEvent('esx_vehicleshop1:sendCategories')
AddEventHandler('esx_vehicleshop1:sendCategories', function (categories)
	Categories = categories
end)

RegisterNetEvent('esx_vehicleshop1:sendVehicles')
AddEventHandler('esx_vehicleshop1:sendVehicles', function (vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	...
	ESX.TriggerServerCallback('esx_vehicleshop1:getCommercialVehicles', function (vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu',
		{
			...
			TriggerServerEvent('esx_vehicleshop1:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('esx_vehicleshop1: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop',
	{
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_vehicleshop1:buyVehicleSociety', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_vehicleshop1:buyVehicle', function(hasEnoughMoney)
									...
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											if Config.EnableOwnedVehicles then
												TriggerServerEvent('esx_vehicleshop1:setVehicleOwned', vehicleProps)
											end
								...
								ESX.TriggerServerCallback('esx_vehicleshop1:buyVehicleSociety', function (hasEnoughMoney)
									...
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											TriggerServerEvent('esx_vehicleshop1:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',
	{
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('esx_vehicleshop1:sellVehicle', model)
				TriggerServerEvent('esx_vehicleshop1:addToList', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('esx_vehicleshop1:soldcar', GetPlayerServerId(closestPlayer), vehicleProps)
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('esx_vehicleshop1:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						TriggerServerEvent('esx_vehicleshop1:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))
						...
						TriggerServerEvent('esx_vehicleshop1:setVehicleForAllPlayers', vehicleProps, Config.Zones.ShopInside.Pos.x, Config.Zones.ShopInside.Pos.y, Config.Zones.ShopInside.Pos.z, 5.0)
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_vehicleshop1:getRentedVehicles', function (vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		elseif data.current.value == 'sold_vehicles' then
			...
			ESX.TriggerServerCallback('esx_vehicleshop1:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_vehicleshop1:getStockItems', function (items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_vehicleshop1:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_vehicleshop1:getPlayerInventory', function (inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_vehicleshop1:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('esx_vehicleshop1:hasEnteredMarker', function (zone)
...
AddEventHandler('esx_vehicleshop1:hasExitedMarker', function (zone)
...
Citizen.CreateThread(function ()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_vehicleshop1:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_vehicleshop1:hasExitedMarker', LastZone)
		end
	end
end)
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					...
					ESX.TriggerServerCallback('esx_vehicleshop1:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					...
					ESX.TriggerServerCallback('esx_vehicleshop1:resellVehicle', function(vehicleSold)
