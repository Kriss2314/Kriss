...
AddEventHandler('esx_basicneeds:onEnergy', function()
	if not IsAnimated then
		...
		ESX.Game.SpawnObject('prop_energy_drink', {
			...
			SetRunSprintMultiplierForPlayer(player, 1.27)
			 local timer = 0
			 while timer < 300 do
...
RegisterNetEvent('esx_basicneeds:gummyBear')
AddEventHandler('esx_basicneeds:gummyBear', function()
	...
	ESX.ShowNotification('Czujesz, że stamina wraca do normalnego poziomu')
end)
